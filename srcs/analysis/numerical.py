...
def numerical_data_analyzer(
    ...
    # Percentiles
    print('-' * 100)
    print(df[column].describe(percentiles=percentiles))
    print('-' * 100)
    
    fig, ax = plt.subplots(1, 3, figsize=(13, 8))

    # Violin plot
    sns.violinplot(data=df, x=target, y=column, ax=ax[0], linewidth=1, legend=False)
    ax[0].set_title('Violin plot', fontsize=11, fontweight='bold')
    ax[0].set_xlabel(target, fontsize=9)
    ax[1].set_ylabel(column, fontsize=9)

    # Box plot
    sns.boxplot(data=df,x=target, y=column, ax=ax[1], linewidth=1, legend=False)
    ax[1].set_title('Box_plot', fontsize=11, fontweight='bold')
    ax[1].set_xlabel(target, fontsize=9)
    ax[1].set_ylabel(column, fontsize=9)

    # Dist plot
    sns.kdeplot(data=df[df[target] == 0], x=column, label='Non-defaulter', color='black', ax=ax[2])
    sns.kdeplot(data=df[df[target] == 1], x=column, label='Defaulter', color='red', ax=ax[2])
